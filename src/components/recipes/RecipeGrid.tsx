  ...
  const filtered = useMemo(
    () =>
      recipes.filter((recipe) => {
        ...
        const matchesSearch = `${recipe.title} ${recipe.description} ${recipe.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div>
      <div className="grid gap-4 md:grid-cols-[1fr_300px] md:items-center">
        <RecipeFilters active={category} categories={["All", ...recipeCategories]} onChange={setCategory} />
